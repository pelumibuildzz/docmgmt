...
const RestoredMemberSuccessModal = ({ onClose, handleBackToTeams }: { onClose: () => void, handleBackToTeams: () => void }) => {
  return (
    <div className="absolute top-0 left-0 w-full h-full bg-black/70 text-[16px] flex justify-center items-center z-50">
      <div className="bg-white w-2/5 rounded-xl shadow-md p-10 flex flex-col space-y-6 border-b-[.5px] justify-center items-center border-gray-300">
        ...
        <span className="text-[14px] text-gray">You have successfully restored “Abidogun Aminat” member access.</span>
        <div className='flex justify-center'>
            <button onClick={handleBackToTeams} className='py-2 px-4 rounded-md w-fit border border-gray-300 hover:bg-gray-50 transition-colors'>
                Back to Teams
            </button>
        </div>
      </div>
...
export default RestoredMemberSuccessModal