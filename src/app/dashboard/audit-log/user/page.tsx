...
const UserAuditlogPage = () => {
  ...
  return (
    <main className="w-full py-8 px-6 flex flex-col space-y-[50px]">
        <div className="flex flex-col space-y-6">
            <div className="flex justify-between">
                <div className="flex space-x-4 rounded-xl border-gray-300 border-[.5px]">
                    <Link href={"/dashboard/audit-log/documents"} className="px-3 py-2 text-gray rounded-l-xl">Documents</Link>
                    <Link href={"/dashboard/audit-log/user"} className="px-3 py-2 bg-primary-light text-primary rounded-r-xl w-max">User</Link>
                </div>
        ...
        <UserAuditTable onViewClick={handleViewClick} />
        {isModalOpen && <UserAuditLogModal isOpen={isModalOpen} onClose={handleCloseModal} />}
    </main>
  )
}

export default UserAuditlogPage